...
- [ ] **Reduce silence tail: `silence_duration_ms` 1200→800** — in `pi/audio/capture.py`, change the `silence_duration_ms` default in `VoiceCapture.__init__()` from 1200 to 800; add `silence_duration_ms` key to `config/settings.yaml` under an `audio:` section and wire it into `AudioConfig` in `shared/config.py` and through to `VoiceCapture`. Expected saving: ~400ms per activation (~17% of Phase B conversational, 10% of tool query). Risk: very short pauses in longer utterances may truncate; 800ms is the safe midpoint.
